...
def motion_process(image_size, motion_angle):  # 图像模糊函数（点模糊函数）
...
def make_blurred(input, PSF, eps):  # 对图像进行模糊
...
def wiener(img, kernel, eps, K = 0.01):  # 维纳滤波，K=0.01
    img_fft = np.fft.fft2(img)

    kernel_fft = np.fft.fft2(kernel) + eps
...
def main():
    ...
    line = cv2.HoughLinesP(ift.astype(np.uint8), 1, np.pi / 180, 20, 0, 25, 0)
    l = []
